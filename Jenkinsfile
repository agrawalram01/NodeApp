		...
		stage("Deploy to k8s"){
			steps{
				...
				sshagent(["k8s-machine"]){
					sh "scp -o StrictHostKeyChecking=no services.yml deployments.yml ubuntu@3.144.229.221:/home/ubuntu/nodeapp/"
					script{
						try{
							sh "ssh ubuntu@3.144.229.221:/home/ubuntu/nodeapp/ kubectl get nodes"
